...
class TPSLPositionTracker:
    ...
    def open_position(
        ...
        if inst not in self._tickets:
            self._tickets[inst] = []
        self._tickets[inst].append(ticket)
    ...
    def close_position(
        ...
        inst = instrument.upper()
        # Closes all tickets for this instrument to match legacy flat-close behavior
        tickets = self._tickets.get(inst, [])
        while tickets:
    ...
    def sync_to_net_position(
        ...
        *,
        ...
        """Synchronize local ticket state with a confirmed broker/backtest fill."""
